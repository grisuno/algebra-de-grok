...
def init_wandb(project_name="grokking-curriculum", config=None):
    """Initialize wandb tracking"""
    wandb.init(
        project=project_name,
        config=config or {},
        name=f"stage_{config.get('stage', 0)}_n{config.get('n_bits', 0)}_d{config.get('d_h', 0)}"
    )

def log_training_step(step, train_acc, test_acc, psi, lc, loss_cls=None, loss_sae=None):
    ...
    if loss_cls is not None:
        metrics["loss_classification"] = loss_cls
    if loss_sae is not None:
        metrics["loss_sae"] = loss_sae
...
class AdaptiveCurriculumTrainer:
    ...
    def calculate_adaptive_params(self, n_bits, d_h, stage):
        """Calculate adaptive parameters according to stage complexity"""
    ...
    def smart_weight_transfer(self, prev_model, new_model, stage):
        """Intelligent weight transfer with padding/interpolation"""
    ...
    def detect_stagnation(self, history, current_lc, d_h, step):
        """Detect if model is stagnant and needs restart"""
        if len(history) < 10:  
            return False, None
    ...
    def train_stage(self, stage, n_bits, d_h, prev_model=None, prev_sae=None):
        """Train individual stage with adaptive parameters"""
        print(f"\n{'='*70}")
        print(f" Stage {stage+1} Adaptive: n_bits={n_bits}, d_h={d_h}")
        print(f"{'='*70}")
        ...
        init_wandb(
            project_name="grokking-curriculum",
            config={
                'stage': stage,
                'n_bits': n_bits,
                'd_h': d_h,
        ...
        print(f"\n{'Step':<8} | {'T-Acc':<6} | {'V-Acc':<6} | {'psi':<6} | {'LC':<6} | {'Status':<12}")
        print("-" * 80)
        ...
        for step in range(1, params['max_steps'] + 1):
            ...
            if step % 2000 == 0 or step == 1:
                ...
                with torch.no_grad():
                    ...
                    log_training_step(step, train_acc, test_acc, psi, lc_n, loss_cls.item(), loss_sae.item())
                ...
                if is_stagnant:
                    print(f"\n[!] Stagnation detected ({reason}) in STEP {step}. Reload Optimizer...")
                    
                    last_accs = [f"{h['test_acc']:.3f}" for h in history[-5:]]
                    print(f"   Last 5 test accuracies: {last_accs}")
        ...
        print(f"\n[x] GROKKING NOT REACHED IN STAGE {stage+1} after {params['max_steps']} steps.")
    ...
    def run_curriculum(self):
        """Execute complete curriculum with automatic adaptation"""
        curriculum = [
            (10, 128),   
            (24, 256),   
            (32, 512),   
        ...
        for stage, (n_bits, d_h) in enumerate(curriculum):
            ...
            if not success:
                print(f"\n[-] CURRICULUM stopped in stage {stage+1}.")
                break
